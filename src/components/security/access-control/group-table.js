...
function createData(group, users, permissionpolicies, validupto) {
  return {
    group, users, permissionpolicies, validupto
  };
}

const rows = [
  createData('edge', 'none', 'none', '31/10/2023'),
  createData('iegroup', 'sita', 'none', '31/05/2024'),
  createData('edge1', 'Ram', 'none', '31/10/2023'),
  createData('iegroups', 'sita', 'none', '31/05/2024'),
  createData('edge', 'none', 'none', '31/10/2023'),
  createData('iegroup', 'sita', 'none', '31/05/2024'),
  createData('edge', 'none', 'none', '31/10/2023'),
  createData('iegroup', 'sita', 'none', '31/05/2024'),
...
function GroupTable() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = rows.map((n) => n.group);
  ...
  const handleClick = (event, group) => {
    ...
    const selectedIndex = selected.indexOf(group);
    let newSelected = [];
    if (selectedIndex === -1) {
      newSelected = newSelected.concat(selected, group);
  ...
  const isSelected = (group) => selected.indexOf(group) !== -1;
          ...
          <TableBody>
            {stableSort(rows, getComparator(order, orderBy))
              ...
              .map((row, index) => {
                ...
                return (
                  <TableRow hover onClick={(event) => handleRowClick(event, row.group)} role="checkbox" aria-checked={isItemSelected} tabIndex={-1} key={row.group} selected={isItemSelected}>
                    <TableCell component='td' padding="checkbox" >
                      <Checkbox color="primary" onClick={(event) => handleClick(event, row.group)} checked={isItemSelected} inputProps={{ 'aria-labelledby': labelId }} />
                    </TableCell>
                    <TableCell component='td' size='small'> {row.group}</TableCell>
                    <TableCell component='td' size='small'>{row.users}</TableCell>
                    <TableCell component='td' size='small'>{row.permissionpolicies}</TableCell>
                    <TableCell component='td' size='small'>{row.validupto}</TableCell>
                  </TableRow>
                );
              })}
          </TableBody>
